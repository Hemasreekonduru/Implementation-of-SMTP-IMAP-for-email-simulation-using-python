...
def objectCreation(email_variable):
   ...
   if email_variable.get().split('@')[-1] =='gmail.com':
...
def login_check_smtp(email_variable, password_variable):
    try:
       print(smtp.login(email_variable.get(),password_variable.get()))
       return 1
    except:
        return 0
...
def login_check_imap(email_variable, password_variable):
    try:
       print(imap.login(email_variable.get(),password_variable.get()))
       return 1
    except:
        return 0
...
def toggle_and_login_check(email_variable, password_variable):

    if email_variable.get() == '' or password_variable.get() == '':
        print('Either empty Username or Password')
        root_info_label.config(text ='Username or Password is Empty')
    else:
        objectCreation(email_variable)
        
    if login_check_smtp(email_variable,password_variable) == 1 and login_check_imap(email_variable, password_variable) == 1:
        toggle()
    else:
...
def send_mail(from_address, to_var, cc_var, bcc_var, subject_var, body_var, attach,
    attachments = []):
...
def put_content(l):
    ...
    for ind, val in enumerate(l[::-1]):
        str_value = ('Date: {}, Subject: {},To: {}, From: {}'.format(val['Date'], val['Subject'], val['To'], val['From']))
        listbox.insert(ind, str_value)
...
def get_attachments(msg):
   ...
   for part in msg.walk():
       ...
       if bool(filename):
          global dir_name
          dir_name =filedialog.askdirectory(initialdir = 'C:')
...
def put_attachment():
   ...
   temp =filedialog.askopenfilenames(initialdir = 'D:\\', title = 'Select Attachment(s)', filetypes = (('Allfiles', '*.*'), ('JPEG files', '*.jpeg')))
...
def compose_mail():
   ...
   compose = Toplevel()
   compose.title('Compose Mail')
   compose.iconbitmap(r'C:\Users\kondu\Downloads\New folder\computer networks\cn project\email.ico')
   ...
   # Creating To Label
   to_label = Label(compose, text = 'To', font= ('', 15))
   to_label.grid(row = 0, column = 0,padx = 10, pady = 10)

   # Creating CC Label
   cc_label = Label(compose, text = 'CC', font= ('', 15))
   cc_label.grid(row = 1, column = 0,padx = 10, pady = 10)

   # Creating BCC Label
   bcc_label = Label(compose, text = 'BCC',font = ('', 15))
   bcc_label.grid(row = 2, column = 0,padx = 10, pady = 10)

   # Creating Subject Label
   subject_label = Label(compose, text ='Subject', font = ('', 15))
   subject_label.grid(row = 3, column = 0,padx = 10, pady = 10)

   # Creating Body Label
   body_label = Label(compose, text = 'Body',font = ('', 15))
   body_label.grid(row = 4, column = 0,padx = 10, pady = 10, sticky = N)

   # Creating To Entry
   to_entry = Entry(compose, font = ('', 10),width = 60)
   to_entry.grid(row = 0, column = 1,padx = 10, pady = 10, sticky = W)

   # Creating CC Entry
   cc_entry = Entry(compose, font = ('', 10),width = 60)
   cc_entry.grid(row = 1, column = 1,padx = 10, pady = 10, sticky = W)

   # Creating BCC Entry
   bcc_entry = Entry(compose, font = ('', 10),width = 60)
   bcc_entry.grid(row = 2, column = 1,padx = 10, pady = 10, sticky = W)

   # Creating Subject Entry
   subject_entry = Entry(compose, font = ('',10), width = 60)
   subject_entry.grid(row = 3, column =1, padx = 10, pady = 10, sticky = W)

   # Creating Body ScrolledText
   scrolled_text = ScrolledText(compose,wrap = WORD, font = ('Verdana', 12))
   scrolled_text.grid(row = 4, column = 1,padx = 10, pady = 10)

   # Creating Attach button
   attachment_button = Button(compose, text ='Attach', fg = 'white', bg = 'blue', font = ('', 10), command = put_attachment)
   attachment_button.grid(row = 5, column =1, sticky = E, padx = 10, pady = 10)

   # Creating Send Button
   send_button = Button(compose, text = 'Send',bg = 'blue', fg = 'white', font = ('', 12), command = lambda: send_mail(root_email_entry.get(),to_entry, cc_entry, bcc_entry, subject_entry, scrolled_text, attachment_label,attachment_list))
   send_button.grid(row = 5, column = 0,padx = 10, pady = 10)

   # Creating Attachment Label
   global attachment_label
   attachment_label = Label(compose, font =('', 10))
...
root.iconbitmap(r'C:\Users\kondu\Downloads\New folder\computer networks\cn project\email.ico')
...
x = (screen_width // 2) - (application_width // 2)
y = (screen_height // 2) - (application_height // 2)
root.geometry(f'{application_width}x{application_height}+{x}+{y}')

# Creating Login Frame
root_frame = Frame(root, bg = 'orange')
...
root_email_label = Label(root_frame, text = 'Email', font = ('', 16), bg = 'orange')
...
root_password_label = Label(root_frame, text = 'Password', font = ('', 16), bg = 'orange')
...
# Creating Info Label
root_info_label = Label(root_frame, text = '', fg = 'red', font = ('', 16), bg = 'orange')
root_info_label.grid(columnspan = 2, padx = 5, pady = 5)

# Creating Mail sending Window
top = Toplevel()
top.withdraw()
top.iconbitmap(r'C:\Users\kondu\Downloads\New folder\computer networks\cn project\email.ico')
